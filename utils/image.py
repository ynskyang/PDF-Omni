...
def imrescale(image: np.ndarray, scale: float) -> np.ndarray:
    ...
    channel_axis = -1 if multi_channel else None
...
def writeImageFloat(image: np.ndarray, tiff_path: str, thumbnail = None):
    ...
    with tifffile.TiffWriter(tiff_path) as tiff:
        write = tiff.write if hasattr(tiff, 'write') else tiff.save
        if thumbnail is not None:
            if not thumbnail.dtype == np.uint8:
                thumbnail = thumbnail.astype(np.uint8)
            write(thumbnail, photometric='RGB', planarconfig='CONTIG')
        if not image.dtype == np.float32:
            image = image.astype(np.float32)
        try:
            write(image, photometric='MINISBLACK', planarconfig='CONTIG',
                  compression='deflate', compressionargs={'level': 9})
        except TypeError:
            write(image, photometric='MINISBLACK', planarconfig='CONTIG',
                  compress=9)

def _toChannelLast(image: np.ndarray) -> np.ndarray:
    if image.ndim > 2 and \
            image.shape[-1] not in (3, 4) and image.shape[-3] in (3, 4):
...
def readImageFloat(tiff_path: str, return_thumbnail = False,
    ...
    try:
        with tifffile.TiffFile(tiff_path) as tiff:
            multi_image = [_toChannelLast(page.asarray()) for page in tiff.pages]
        num_read_images = len(multi_image)
        if num_read_images == 0:
            raise Exception('No images found.')
        elif num_read_images == 1:
            image = multi_image[0].squeeze()
