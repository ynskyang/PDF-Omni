...
def loadDBConfigs(dbname: str, dbpath: str, opts: Edict) \
    ...
    for k in config['config'].keys():
        opts[k] = config['config'][k]
    opts.min_depth = opts.omnimvs_sweep_min_depth
    for k in config['dataset'].keys():
        opts[k] = config['dataset'][k]
    ...
    for i in range(4):
        ...
        ocam.invalid_mask = readImage(mask_file).astype(bool)
        ocams.append(ocam)

    func = globals().get(f'__load_train_{name}')
    try:
        if func is not None:
            opts = func(opts, dbpath)
            LOG_INFO('Found "%s" training configs' % (dbname))
            opts.dtype = 'gt'
        else:
            LOG_INFO('Training configs not found "%s"' % (dbname))
    except Exception as exc:
        LOG_ERROR('Failed to load "%s" training configs: %s' % (dbname, exc))
    finally:
        return opts, ocams


def __load_train_sunny(opts, dbpath=None):
...
def __load_train_omnithings(opts, dbpath=None):
...
def __load_train_omnihouse(opts, dbpath=None):
