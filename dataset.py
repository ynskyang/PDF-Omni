...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dbname: str, db_opts=None, load_lut=True, train=True, db_root='../omnidata'):
        ...
        opts.lut_fmt = 'PDF_Omni_ds%d_lt_(%d,%d,%d).hwd'
    ...
    def loadImages(self, sample_ref, out_raw_imgs=False, use_rgb=False):
        fidx = int(sample_ref)
        imgs = []
        raw_imgs = []
        for i in range(4):
            ...
            raw_imgs.append(I)
        if sample_ref in self.train_idx:
            raw_imgs = self.augmentor(raw_imgs)
        for cam_idx, I in enumerate(raw_imgs):
            if not use_rgb and len(I.shape) == 3 and I.shape[2] == 3: 
                I = rgb2gray(I, channel_wise_mean=True)
            I = normalizeImage(I, self.ocams[cam_idx].invalid_mask)
            if len(I.shape) == 2:
    ...
    def loadGTInvdepthIndex(self, sample_ref, remove_gt_noise=True,
                            morph_win_size=5):
        fidx = int(sample_ref)
        h, w = self.equirect_size
    ...
    def loadSample(self, sample_ref, read_input_image=True, varargin=None):
        opts = Edict()
        ...
        imgs, raw_imgs = [], []
        if read_input_image:
            imgs, raw_imgs = self.loadImages(sample_ref, True, use_rgb=self.use_rgb)
        gt, valid = [], []
        if self.dtype == 'gt':
            gt = self.loadGTInvdepthIndex(sample_ref,
                opts.remove_gt_noise, opts.morph_win_size)
            valid = np.logical_and(
                gt >= 0, gt <= self.num_invdepth).astype(bool)
    ...
    def writePointCloud(self, pano_rgb: np.ndarray, invdepth: np.ndarray, path: str) -> None:
        if o3d is None:
            raise ImportError('open3d is required to save point clouds')
        invdepth = toNumpy(invdepth)
        depth = 1 / invdepth.reshape((1, -1))
        val = depth[0] < 50
        P = depth * self.rays
...
class MultiDataset(Dataset):
    def __init__(self, dbnames: list, db_opts=None, load_lut=True, train=True,
        ...
        self.dbnames = [dbname.lower() for dbname in dbnames]
